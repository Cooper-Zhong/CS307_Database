...
//        Properties prop = loadDBUser();
        Properties prop = loadGaussUser(); // load user info from file
        openGauss(prop); // connect to openGauss
        Controller controller = new Controller(con, stmt);
        controller.handle();
    ...
    private static void openGauss(Properties prop) {
        try {
            Class.forName("org.postgresql.Driver");
        } catch (Exception e) {
            System.err.println("Cannot find the Postgres driver. Check CLASSPATH.");
    ...
    private static Properties loadGaussUser() {
        Properties properties = new Properties();
        try {
